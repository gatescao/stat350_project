...
```{r}
reduced <- lm(medv ~ crim + zn + chas + nox + rm + dis + rad + tax + ptratio + black + lstat + I(rm^2) + I(lstat^2) + I(crim^2) + I(ptratio^2) + I(zn^2) + I(black^2) + I(tax^2), data = Boston_dropoutlier)
anova(reduced, fit)
```
```{r}
summary(reduced)
mean((Boston_dropoutlier$medv - reduced$fitted.values)^2)
#MSE increases, so not use reduced model.
```
